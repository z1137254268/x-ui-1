...
download_xui(){
    ...
    if [ $# == 0 ]; then
        last_version=$(curl -Ls "https://api.github.com/repos/z1137254268/x-ui-1/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-$(archAffix).tar.gz https://github.com/z1137254268/x-ui-1/releases/download/${last_version}/x-ui-linux-$(archAffix).tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/z1137254268/x-ui-1/releases/download/${last_version}/x-ui-linux-$(archAffix).tar.gz"
    ...
    wget -N --no-check-certificate https://raw.githubusercontents.com/z1137254268/x-ui-1/main/x-ui.sh -O /usr/bin/x-ui
